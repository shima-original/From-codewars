...
Output should be the length of the longest word, as a number.There will only be one 'longest' word.  */
function findLongest(str) {
  ...
  for (let i = 0; i < spl.length; i+=1) {
    let splWord = spl[i].split("");
    if (splWord.length > longest) {
      longest = splWord.length;
    }
  }
  return longest;
}
